...
class Ship
{
     ...
     *
     * @OneToMany(targetEntity="Payload", mappedBy="ship", cascade={"persist", "remove"}, orphanRemoval=true)
     *
     * @Assert\NotNull
     * @Assert\Valid
     *
    ...
    {
        return $this->class ? $this->class->getMountPointCapacity() : 0;
    }
    ...
    public function getEquipmentSummary()
    {
        ...
        foreach ($this->mountPoints as $mountPoint) {
            $equipment = $mountPoint->getEquipment();

            if (null !== $equipment) {
                if (!isset($summary[$equipment])) {
                    $summary[$equipment] = 0;
                }
    ...
    public function getPayloadSummary()
    {
        ...
        foreach ($this->payload as $payload) {
            $good = $payload->getGood();

            if (null !== $good && !isset($summary[$good])) {
                $summary[$good] = 0;
            }

            $summary[$good] += $payload->getMass();
        }
